...
[[ ! -e Starter.xcodeproj ]] || { echo "error: remove the generated Starter.xcodeproj first (make clean)" >&2; exit 1; }
...
while IFS= read -r -d '' file; do
  ...
  -not -path './dist/*' -not -path './scripts/rename.sh' -print0)
...
echo "Renamed Starter to $new_name ($new_bundle_id). Run make validate and make generate."
